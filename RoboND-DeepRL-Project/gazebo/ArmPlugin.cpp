...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		*/
		...
		//bool collisionItemCheck=(strcmp(contacts->contact(i).collision1().c_str(),COLLISION_ITEM)==0);//task1
		bool collisionItemCheck=(strcmp(contacts->contact(i).collision2().c_str(),COLLISION_POINT)==0); //task2
		if(collisionItemCheck){			
			//rewardHistory=REWARD_WIN;//task1
			rewardHistory=REWARD_WIN*5;
			newReward=true;
			endEpisode=true;
			return;
		}else{
			rewardHistory=REWARD_LOSS*0.08f;
			newReward=true;
			endEpisode=false;
		}
		...
		*/ 
		...
		if(!GroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
				...
				avgGoalDelta  = ALPHA*avgGoalDelta+distDelta*(1-ALPHA);
